...
sp::plot(ukmap,xlim=xRa, ylim=yRa,col="darkgreen",border="darkgreen", axes=T, yaxt="n",
         ...
         main="Curlew 1") # (manually for each individual)e.g. main=paste("May_",names(grd_rank_birds[2]), sep="")
...
landuse <- projectRaster(landuse, crs =("+proj=tmerc +lat_0=49 +lon_0=-2 +k=0.9996012717 +x_0=400000 +y_0=-100000 +ellps=airy +datum=OSGB36 +units=m +no_defs"), method = "ngb") # method nearest neighbour for categorical raster values (opposed to bilinear interpolation)


## clean GPS (use BTOTT?)

## Convert to 'amt' track (using Movebank headers)
trk <- make_track(data, .x = location.long, .y = location.lat, .t = timestamp, id = individual.local.identifier, crs = CRS("+init=epsg:4326"))
...
## Transform to BNG
trk <- transform_coords(trk, CRS("+proj=tmerc +lat_0=49 +lon_0=-2 +k=0.9996012717 +x_0=400000 +y_0=-100000 +ellps=airy +datum=OSGB36 +units=m +no_defs"))
...
theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) # rotated x axis labels for individual plots
